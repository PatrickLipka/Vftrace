PROGRAM ireduce_scatter_block_inplace
   ...
   IF (COMMAND_ARGUMENT_COUNT() < 1) THEN
      WRITE(UNIT=OUTPUT_UNIT, FMT="(A)") "./ireduce_scatter_block_inplace <msgsize in integers>"
      STOP 1
   END IF
   ...
   ! Message cycle
   CALL MPI_Ireduce_scatter_block(MPI_IN_PLACE, rbuffer, nints, MPI_INTEGER, &
...
END PROGRAM ireduce_scatter_block_inplace
