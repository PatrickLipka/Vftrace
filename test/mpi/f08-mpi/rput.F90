PROGRAM rput
   ...
   TYPE(MPI_Status), DIMENSION(:), ALLOCATABLE :: mystatuses
   TYPE(MPI_Request), DIMENSION(:), ALLOCATABLE :: myrequests
   ...
   IF (COMMAND_ARGUMENT_COUNT() < 1) THEN
      WRITE(UNIT=OUTPUT_UNIT, FMT="(A)") "./rput <msgsize in integers>"
      STOP 1
   END IF
   ...
   IF (my_rank == 0) THEN
      ...
      DO targetrank = 1, comm_size - 1
         ...
         CALL MPI_Rput(srbuffer, nints, MPI_INTEGER, targetrank, targetdisp, &
                       nints, MPI_INTEGER, window, myrequests(targetrank), ierr)
      END DO
      ALLOCATE(mystatuses(comm_size-1))
