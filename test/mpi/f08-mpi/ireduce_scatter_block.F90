PROGRAM ireduce_scatter_block
   ...
   IF (COMMAND_ARGUMENT_COUNT() < 1) THEN
      WRITE(UNIT=OUTPUT_UNIT, FMT="(A)") "./ireduce_scatter_block <msgsize in integers>"
      STOP 1
   END IF
   ...
   ALLOCATE(sbuffer(comm_size*nints))
   DO irank = 0, comm_size-1
      DO i = 0, nints-1
         sbuffer(irank*nints+i+1) = irank
      END DO
   END DO
   ...
   ! Message cycle
   CALL MPI_Ireduce_scatter_block(sbuffer, rbuffer, nints, MPI_INTEGER, &
...
END PROGRAM ireduce_scatter_block
