PROGRAM rget_accumulate
   ...
   IF (COMMAND_ARGUMENT_COUNT() < 1) THEN
      WRITE(UNIT=OUTPUT_UNIT, FMT="(A)") "./rget_accumulate <msgsize in integers>"
      STOP 1
   END IF
   ...
   IF (my_rank == 0) THEN
      ...
      DO irank = 1, comm_size - 1
         ...
         CALL MPI_Rget_accumulate(originbuffer, nints, MPI_INTEGER, & ! origin info
                                  resultbuffer, nints, MPI_INTEGER, & ! result info
                                  irank, targetdisp, nints, MPI_INTEGER, & ! target info
                                  MPI_SUM, window, myrequest, ierr)
