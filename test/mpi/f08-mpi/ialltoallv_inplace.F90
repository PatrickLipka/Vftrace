PROGRAM ialltoallv_inplace
   ...
   IF (COMMAND_ARGUMENT_COUNT() < 1) THEN
      WRITE(UNIT=OUTPUT_UNIT, FMT="(A)") "./ialltoallv_inplace <msgsize in integers>"
      STOP 1
   END IF
   ...
   ! Messaging
   CALL MPI_Ialltoallv(MPI_IN_PLACE, scounts, sdispls, MPI_DATATYPE_NULL, &
...
END PROGRAM ialltoallv_inplace
