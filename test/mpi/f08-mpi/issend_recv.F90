PROGRAM issend_recv
   ...
   IF (COMMAND_ARGUMENT_COUNT() < 1) THEN
      WRITE(UNIT=OUTPUT_UNIT, FMT="(A)") "./issend_recv <msgsize in integers>"
      STOP 1
   END IF
   ...
   ELSE
      ...
      CALL MPI_Issend(sbuffer, nints, MPI_INTEGER, 0, my_rank, MPI_COMM_WORLD, myrequest, ierr)
