PROGRAM ialltoallw
   ...
   TYPE(MPI_Datatype), DIMENSION(:), ALLOCATABLE :: stypes
   ...
   IF (COMMAND_ARGUMENT_COUNT() < 1) THEN
      WRITE(UNIT=OUTPUT_UNIT, FMT="(A)") "./ialltoallw <msgsize in integers>"
      STOP 1
   END IF
