PROGRAM bsend_recv
   ...
   IF (COMMAND_ARGUMENT_COUNT() < 1) THEN
      WRITE(UNIT=OUTPUT_UNIT, FMT="(A)") "./bsend_recv <msgsize in integers>"
      STOP 1
   END IF
   ...
   ELSE
      ...
      CALL MPI_Bsend(sbuffer, nints, MPI_INTEGER, 0, my_rank, MPI_COMM_WORLD, ierr)
   END IF

   CALL MPI_Buffer_detach(buffer_addr, bufsize, ierr)
